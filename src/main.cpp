...
namespace WebSocket
{
    ...
    std::string GetOrigin(std::string& buffer);
...
std::string WebSocket::GetOrigin(std::string& buffer)
{
	size_t pos = buffer.find("Origin: ") + 8;
...
// Sec-WebSocket-Key1
uint32_t WebSocket::GetKey1(std::string& buffer)
{
...
// Sec-WebSocket-Key2
uint32_t WebSocket::GetKey2(std::string& buffer)
{
...
std::string WebSocket::GetHost(std::string& buffer)
{
...
std::vector<uint8_t> WebSocket::CreateServerHandshake(std::string& buffer)
{
...
std::string WebSocket::CreateClientKey(std::string& buffer)
{
...
public:
	...
	void RemoveClient(std::vector<Client::ptr>::iterator iter);
...
WebSocket::Client::Client(boost::asio::io_service& io_service)
	: socket_(io_service), handshake_(false) { }

WebSocket::Client::Client(boost::asio::io_service& io_service, server_raw_ptr const server)
	: socket_(io_service), server_(server), handshake_(false) { }

WebSocket::Client::~Client() { }

WebSocket::Client::ptr WebSocket::Client::Create(boost::asio::io_service& io_service)
{
...
boost::asio::ip::tcp::socket& WebSocket::Client::Socket()
{
	return socket_;
}

void WebSocket::Client::Start()
{
	StartRead();
}

void WebSocket::Client::StartRead()
{
...
std::vector<uint8_t> WebSocket::Client::Wrap(std::string& data)
{
...
std::vector<uint8_t> WebSocket::Client::Wrap(std::stringstream& data)
{
	std::vector<uint8_t> tmp_data;
...
void WebSocket::Client::HandleRead(const boost::system::error_code& error,
...
WebSocket::Server::Server(boost::asio::io_service& io_service, const char* host, unsigned short port,
	client_message_handler client_message_handler_cb)
...
std::vector<Client::ptr>& WebSocket::Server::client_list()
{
	return client_list_;
}

void WebSocket::Server::RemoveClient(std::vector<Client::ptr>::iterator iter)
{
	io_service_.post(boost::bind(&std::vector<Client::ptr>::erase, &client_list_,
		iter));
}

void WebSocket::Server::StartAccept()
{
...
void WebSocket::Server::HandleAccept(const boost::system::error_code& error)
{
	if (!error)
	{
...
int main(int arg, char** argv)
{
	...
	WebSocket::Server s(io_service, argv[1], atoi(argv[2]), 0);
